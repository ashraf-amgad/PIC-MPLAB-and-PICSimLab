...
void main() {
    ...
    uint8_t EEPROM_Value = External_EEPROM_Read(0xA0 , 0x00);


    if (EEPROM_Value == 7) {
        EEPROM_Value = External_EEPROM_Read(0xA0 , 0x01);
        Seconds_Counter = EEPROM_Value;

        EEPROM_Value = External_EEPROM_Read(0xA0 , 0x02);
        Minute_Counter = EEPROM_Value;
    } else {
        Seconds_Counter = 0;
        Minute_Counter = 0;
    }
    ...
    while (1) {
        ...
        if (RB4 == 0) {
            while (RB4 == 0);
            External_EEPROM_Write(0xA0, 0x00 , 0x07);
            External_EEPROM_Write(0xA0, 0x01 , Seconds_Counter);
            External_EEPROM_Write(0xA0, 0x02 , Minute_Counter);
        }
        ...
        if (count > 0x2ffff) {
            ...
            Minute_Counter = 0;
            count=0;
        } else if ((count > 0x00) && (count < 0x2ffff) ) {
            Seconds_Counter++;
            count=0;
        }
