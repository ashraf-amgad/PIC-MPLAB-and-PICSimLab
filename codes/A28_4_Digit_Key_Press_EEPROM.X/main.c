...
#define Counter_Increment_Reset_Key_Direction     TRISBbits.TRISB3
#define Counter_Increment_Reset_Key                     RB3
...
void main() {
    ...
    Counter_Increment_Reset_Key_Direction = 1;
    ...
    uint8_t EEPROM_Value = External_EEPROM_Read(Device_Address , 0x00);


    if (EEPROM_Value == 7) {
        EEPROM_Value = External_EEPROM_Read(Device_Address , 0x01);
        Seconds_Counter = EEPROM_Value; // Global Counter Variable

        EEPROM_Value = External_EEPROM_Read(Device_Address , 0x02);
    ...
    while (1) {
        ...
        if (Store_Key == 0) {
            while (Store_Key == 0);
            External_EEPROM_Write(Device_Address, 0x00 , 0x07);
            External_EEPROM_Write(Device_Address, 0x01 , Seconds_Counter);
            External_EEPROM_Write(Device_Address, 0x02 , Minute_Counter);
        }

        if (Counter_Increment_Reset_Key == 0) {
            while (Counter_Increment_Reset_Key == 0) {
                count++;
            }
        }
