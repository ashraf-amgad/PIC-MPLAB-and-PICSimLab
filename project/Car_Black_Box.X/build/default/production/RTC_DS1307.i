...
uint8_t RTC_DS1307_Read(uint8_t word_Address) {
    ...
    I2C_Start();
    temp_Device_Address = 0xD0 & ~(0x01);
    I2C_Write(temp_Device_Address);
    I2C_Write(word_Address);
    I2C_Start();
    temp_Device_Address = 0xD0 | (0x01);
    I2C_Write(temp_Device_Address);
...
void RTC_DS1307_Get_Date(RTC_Date_Typedef* Date_Structure) {
    ...
    temp.Seconds = RTC_DS1307_Read(0x00);
    temp.Minutes = RTC_DS1307_Read(0x01);
    temp.Hours = RTC_DS1307_Read(0x02);
    if ((temp.Hours & 0b00100000) != 0) {
        Date_Structure->am_Pm = 1;
    } else {
        Date_Structure->am_Pm = 0;
    }
    temp.Hours &= 0x1f;
    temp.DOW = RTC_DS1307_Read(0x03);
    Date_Structure->DOW_String = RTC_DS1307_DOW_To_String(temp.DOW);
    temp.Day = RTC_DS1307_Read(0x04);
    temp.Month = RTC_DS1307_Read(0x05);
    temp.Year = RTC_DS1307_Read(0x06);
